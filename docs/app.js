...
// ─────────────────────────────────────────────────────────────────────────────
// System Console — collapsible debug/event log panel
// ─────────────────────────────────────────────────────────────────────────────
...
function consoleLog(tag, msg) {
  ...
  tagEl.className = `console-tag tag-${tag}`;
  ...
  // Trim old lines to keep memory lean
  while (consoleOutput.children.length > CONSOLE_MAX_LINES) {
...
const cInfo  = (msg) => consoleLog('info',  msg);
const cOk    = (msg) => consoleLog('ok',    msg);
const cWarn  = (msg) => consoleLog('warn',  msg);
const cError = (msg) => consoleLog('error', msg);
const cWin   = (msg) => consoleLog('win',   msg);

// Toggle visibility of the console output body
consoleToggleBtn.addEventListener('click', () => {
  const collapsed = consoleOutput.classList.toggle('collapsed');
  consoleToggleBtn.textContent = collapsed ? '▶' : '▼';
...
consoleClearBtn.addEventListener('click', () => {
  ...
  cInfo('Console cleared.');
});

// Header button scrolls to and opens the console
...
// Startup message
cInfo('AI Ring v1 — System Console ready.');
...
async function checkServerMode() {
  cInfo('Checking server mode…');
  try {
    const res = await fetch(`${API_BASE}/api/models`);
    if (!res.ok) {
      cWarn(`Server responded ${res.status} — falling back to TRAINING MODE.`);
    ...
    if (data.demoMode) {
      cWarn('No API keys configured — TRAINING MODE active.');
      demoBadge.classList.remove('hidden');
    } else {
      const liveCount = data.configured ? Object.values(data.configured).filter(Boolean).length : 0;
      cOk(`Backend connected. ${liveCount} model(s) LIVE.`);
    }
    applyModelStatus(data.configured || null);
  } catch (_) {
    // No backend (static hosting / GitHub Pages) — run fully client-side
    cWarn('No backend detected — running client-side TRAINING MODE.');
    demoBadge.classList.remove('hidden');
    applyModelStatus(null);
  }
}
...
submitBtn.addEventListener('click', async () => {
  ...
  cInfo(`Battle started — prompt: "${prompt.length > 60 ? prompt.slice(0, 60) + '…' : prompt}"`);
  ...
  try {
    ...
    if (backendAvailable) {
      cInfo('Sending request to backend /api/compete…');
      ...
      data = await res.json();
      cOk(`Backend responded. Mode: ${data.results.every((r) => r.isDemo) ? 'TRAINING' : 'LIVE'}`);
    } else {
      cInfo('Running local competition (TRAINING MODE)…');
    ...
    sorted.forEach((r) => {
      const tag = r.isWinner ? 'win' : 'info';
      const badge = r.isWinner ? '👑 WINNER ' : '';
      consoleLog(tag, `${badge}${r.name}: score=${r.score} latency=${r.latencyMs}ms${r.isDemo ? ' [training]' : ' [live]'}`);
    });
    ...
    renderResults(data);
    addToHistory(data);
    cInfo('Battle complete. Results rendered.');
  } catch (err) {
    cError(`Battle error: ${err.message}`);
    stopBattleSequence();
    stopThinkingBubbles();
    disperseAgents();
    resetAgents();
